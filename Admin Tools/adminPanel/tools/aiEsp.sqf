			...
			onEachFrame {
				_l49 = nearestobjects[player, ["CAManBase"], 2000]; {
					...
					if (!(isPlayer _x) && ((player distance _x) < 2000)) then {
						drawIcon3D["", [0.3,0.3,0.5,1], [visiblePosition _x select 0, visiblePosition _x select 1, (getPosATL _x select 2) + 1], 0.1, 0.1, 45, (format["%2 : %1m", round(player distance _x), name _x]), 1, 0.04, "TahomaB"]
					};
				}
				foreach allUnits;
			};
